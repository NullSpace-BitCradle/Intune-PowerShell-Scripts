...
try
{
    ...
    if ($WhatIf) {
        Write-Log "What if: Would update Reg key scforceoption with value 1"
    }
    else {
...
try
{
    ...
    if ($WhatIf) {
        Write-Log "What if: Would update Reg key ScRemoveOption with value 1"
    }
    else {
...
try
{
    ...
    if ($WhatIf) {
        Write-Log "What if: Would update Reg key Start with value 2 (Automatic)"
    }
    else {
...
if (-not $WhatIf -and -not $KeepLogs) {
    $logFilePath = Join-Path $Log_Path "win32-Activate_SmartCardLogon_Enforcement.log"
