...
<#
.SYNOPSIS
    Disables smart card logon enforcement on Windows devices.

.DESCRIPTION
    This script disables smart card logon enforcement by modifying registry keys that control
    smart card authentication behavior. It sets registry values to allow password-based logon
...
.EXAMPLE
    .\Disable-SmartCardLogonEnforcement.ps1 -KeepLogs
...
# Section 1: Disable Smart Card Logon Enforcement
# This section modifies the registry to allow password-based logon instead of requiring smart cards
...
# Section 2: Disable Auto-lock upon Smart Card Removal
# This section modifies the registry to prevent automatic screen lock when smart card is removed
...
# Section 3: Disable Smart Card Policy Service
# This section disables the Smart Card Policy Service by setting its startup type to Disabled
...
try
{
    ...
    if ($WhatIf) {
        ...
        try {
            ...
            Write-Log "Service startup type set to Disabled using Set-Service"
        }
        catch {
